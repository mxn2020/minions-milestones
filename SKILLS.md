...
description: Agent skills for working with Minions Milestones MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-milestones data.
---

# Minions Milestones Agent Skills

Skills for agents operating on the `minions-milestones` toolbox.
...
```bash
# TypeScript
pnpm add @minions-milestones/sdk
...
The `milestones` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-milestones/sdk/schemas';
...
```python
from minions_milestones.schemas import custom_types
...
- This agent only writes to `minions-milestones` — it reads from other toolboxes but never writes to them
